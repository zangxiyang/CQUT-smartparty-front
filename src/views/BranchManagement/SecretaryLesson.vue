  ...
  <select-container-layout v-model="active" title="党支部" @change="selectContainerClick">
    <el-row :gutter="20" type="flex">
      <el-col v-for="item in listArr" :key="item.id" :span="12">
        <video-articles-item :list="item" style="margin: 0 auto; margin-bottom: 20px" @click="click" tag="书记讲党课"/>
      </el-col>
    </el-row>
...
export default defineComponent({
  ...
  setup(){
    ...
    const loadData = (orgId = 1,page= 1): void=>{
      listArr.value = []
      BranchWorkService.getPartyWorkList(orgId,ArticleType.SECRETARY_LESSON,page,6).then(res=>{
        console.log(res)
